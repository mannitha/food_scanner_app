...
def estimate_height_with_manual_scale(image, scale_pts):
    ...
    with mp_pose.Pose(static_image_mode=True) as pose:
        ...
        if not results.pose_landmarks:
            return None, "❌ Body landmarks not detected."
...
def run_height_estimator(pil_image):
    st.markdown("🟢 **Mark two points on the steel scale (top and bottom).**")
    ...
    if canvas_result.json_data and len(canvas_result.json_data["objects"]) == 2:
        ...
        with st.spinner("Estimating height..."):
            height_cm, error = estimate_height_with_manual_scale(cv_img.copy(), points)
        ...
        st.info("Please mark **exactly two points** to measure scale.")
        return None
